...
means = [means_all(9); means_all(10); means_all(3); means_all(7); means_all(1); means_all(5); means_all(4); means_all(8)];%mean(data, 1);
stds = [stds_all(9); stds_all(10); stds_all(3); stds_all(7); stds_all(1); stds_all(5); stds_all(4); stds_all(8)];

% means = [means(1) means(2) means(3) means(4) means(5) means(6)];
% stds = [stds(1) stds(2); stds(3) stds(4); stds(5) stds(6)];

xticks = [1 3 5 6 8 9 11 12];
b = bar(xticks, means, 1)

hold on

for i = [3, 5, 7]
    text(xticks(i), means(i) + stds(i) + 0.1, "ASC", 'FontName', 'helvetica', 'FontSize', fontsize-2, 'HorizontalAlignment', 'center')
end
...
set(gca,'XTick', [1 3 5.5 8.5, 11.5], 'xticklabel',{'RNN', 'LSTM', 'HOM', 'HETI', 'HETL'}, 'FontName', 'helvetica', 'FontSize', fontsize);
