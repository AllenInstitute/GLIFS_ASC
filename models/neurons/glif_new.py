...
class BNNC(nn.Module):
	...
	def __init__(self, input_size, hidden_size, bias = True, dt=0.05):
		...
		self.asc_amp = Parameter(torch.ones((self.num_ascs,1,hidden_size), dtype=torch.float), requires_grad=True)
		self.ln_asc_k = Parameter(torch.ones((self.num_ascs, 1, hidden_size), dtype=torch.float), requires_grad=True)
		self.asc_r = Parameter(torch.ones((self.num_ascs,1,hidden_size), dtype=torch.float), requires_grad=True)		
